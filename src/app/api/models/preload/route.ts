...
import { z } from 'zod'
import { 
  PreloadRequestSchema, 
  getPreloadingService
} from '@/lib/model-preloading'
import { 
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (request.type === 'workflow') {
      ...
      const priorities = requirements.map(req => 
        calculateModelPriority(req.name, mockEvents, [request.workflowSlug])
      )
    ...
    return NextResponse.json(
      { error: 'Failed to process preload request', details: message }, 
      { status: 500 }
    )
  }
}