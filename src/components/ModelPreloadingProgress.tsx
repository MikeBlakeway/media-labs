...
export function ModelPreloadingProgress({ 
  ...
  if (workflowSlug && preloading.workflowStatus) {
        ...
        <div className="flex items-center gap-2 text-sm">
          {status.readyNow ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <span>Preparing {status.pendingModels.length} models</span>
              {status.estimatedReadyTime && (
                <span className="text-gray-500">
                  (ready in {formatReadyTime(status.estimatedReadyTime)})
                </span>
              )}
            </div>
          )}
    ...
    return (
      <div className="rounded-xl border p-4 bg-gradient-to-r from-blue-50 to-purple-50">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-gray-900">Model Preloading</h3>
          {showActions && (
            ...
            <div className="space-y-2">
              {status.requiredModels.map((model) => (
                <div key={`${model.type}:${model.name}`} className="flex items-center gap-3">
                  ...
                  <span className="flex-1 text-sm font-mono truncate">{model.name}</span>
        ...
        <div className="flex items-center gap-2 text-sm">
          {queue.summary.activeDownloads > 0 && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-500">
                ({Math.round(queue.summary.overallProgress * 100)}%)
              </span>
            </div>
          )}
          {queue.summary.queuedDownloads > 0 && (
            <span className="text-gray-600">
              +{queue.summary.queuedDownloads} queued
            </span>
          )}
        </div>
      )
        ...
        {queue.queue.active.length > 0 && (
          <div className="space-y-2">
            ...
            {queue.queue.active.map((model) => (
              <div key={`${model.modelType}:${model.modelName}`} className="flex items-center gap-3 p-2 bg-blue-50 rounded-lg">
                <ModelStatusIcon status={model.status} />
