...
export class ModelPreloadingService {
  ...
  async queuePreload(request: PreloadRequest): Promise<{ success: boolean; queued: string[]; skipped: string[] }> {
    ...
    const currentQueueSize = Array.from(this.queue.values()).filter(
      status => status.status === 'queued' || status.status === 'downloading'
    ).length
  ...
  getPreloadStatus(): PreloadQueue {
    ...
    const totalProgress = totalModels > 0 ? statuses.reduce((sum, s) => sum + s.progress, 0) / totalModels : 0

    // Estimate completion time
    const remainingModels = active.length + queued.length
    const estimatedCompletion =
      remainingModels > 0 && active.length > 0 ? this.estimateCompletionTime(active) : undefined

    return {
      active: active.sort((a, b) => b.priority - a.priority),
      queued: queued.sort((a, b) => b.priority - a.priority),
      completed: completed.sort((a, b) => new Date(b.endTime || 0).getTime() - new Date(a.endTime || 0).getTime()),
      failed: failed.sort((a, b) => new Date(b.endTime || 0).getTime() - new Date(a.endTime || 0).getTime()),
      totalProgress,
      estimatedCompletion
    }
  }
  ...
  async cancelPreload(modelNames: string[]): Promise<{ success: boolean; cancelled: string[] }> {
    ...
    for (const [key, status] of this.queue) {
      if (modelNames.includes(status.modelName) && ['queued', 'downloading'].includes(status.status)) {
  ...
  /**
   * Download a single model from B2 to RunPod volume
   */
  private async downloadModel(key: string, status: PreloadStatus): Promise<void> {
    try {
      // Call volume worker to download from B2
      console.log(`[ModelPreloading] Starting B2 download for ${status.modelName}`)

      const startTime = Date.now()
      status.status = 'downloading'
      status.progress = 0

      // Make API call to volume worker
      ...
      if (!response.ok) {
        ...
        throw new Error(`Volume worker API error: ${response.status} ${errorText}`)
      }

      const result = await response.json()

      if (!result.ok) {
        throw new Error(result.error || 'Volume worker operation failed')
      }
      ...
      status.progress = 1
      status.endTime = new Date().toISOString()
      status.downloadSpeed = result.averageSpeed || 0

      // Record model usage for volume management
      const modelSizeBytes = result.totalBytes || 0
      await volumeManagement.recordModelUsage(status.s3Key, status.modelType, modelSizeBytes)

      console.log(`[ModelPreloading] Successfully downloaded ${status.modelName} in ${downloadTimeMs}ms`)
    } catch (error) {
  ...
  private estimateCompletionTime(activeDownloads: PreloadStatus[]): string {
    ...
    const maxTimeRemaining = Math.max(...activeDownloads.map(d => d.estimatedTimeRemaining || 0))
